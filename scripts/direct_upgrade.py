...
# Infinite deterministic chunk streaming with persistent edits.
...
'''
if '.modeSelect{' not in s:s=s.replace('</style>',css+'</style>',1)
if 'id="controlSettings"' not in s:s=s.replace('<button id="pauseBtn">Ⅱ</button>','<button id="pauseBtn">Ⅱ</button><button id="controlSettings" title="تنظیم دکمه‌ها">⚙</button>')
controls='''<div id="controlsModal"><div class="controlsBox"><h2>⚙ تنظیم دکمه‌ها</h2><p>کلیدهای کامپیوتر و کنترل حرکت گوشی را تنظیم کن.</p><div class="controlGrid"><label class="controlRow"><span>جلو</span><input id="bindForward" value="Z" maxlength="12"></label><label class="controlRow"><span>چپ</span><input id="bindLeft" value="Q" maxlength="12"></label><label class="controlRow"><span>عقب</span><input id="bindBack" value="S" maxlength="12"></label><label class="controlRow"><span>راست</span><input id="bindRight" value="D" maxlength="12"></label><label class="controlRow"><span>پرش</span><input id="bindJump" value="Space" maxlength="12"></label><label class="controlRow"><span>Inventory</span><input id="bindInv" value="E" maxlength="12"></label></div><button class="mcbtn green" id="saveControls">ذخیره</button><button class="mcbtn" id="resetControls">پیش‌فرض</button><button class="mcbtn" id="closeControls">بستن</button></div></div>
'''
if 'id="controlsModal"' not in s:s=s.replace('<div id="rotate">',controls+'<div id="rotate">',1)
...
s=s.replace('if((keys.Space||keys.KeyJ)&&player.onGround){player.vel.y=7;player.onGround=false}\n player.vel.y-=19*dt;player.onGround=false;','if(gameMode==="creative"){player.vel.y=0;if(mainmetKeyDown("jump"))player.feet.y+=8*dt;if(keys.ShiftLeft||keys.ShiftRight)player.feet.y-=8*dt;player.onGround=false}else{if((mainmetKeyDown("jump")||keys.KeyJ)&&player.onGround){player.vel.y=7;player.onGround=false}player.vel.y-=19*dt;player.onGround=false;}')
